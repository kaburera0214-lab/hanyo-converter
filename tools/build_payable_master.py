...
def main():
    ...
    # カード払いなど、口座マスタに無いが請求書取込・突合の対象にしたい取引先
    rows.append({
        "会社名": "セルタン", "別名": "", "NE仕入先cd": "c004",
        "支払区分": "カード払い", "科目": "仕入", "支払方法": "カード", "支払日": "",
        "銀行": "", "支店": "", "銀行番号": "", "支店番号": "", "預金種目": "",
        "口座番号": "", "受取人口座名": "", "顧客番号": "", "固定額": "",
        "除外フラグ": "", "ルール": "", "支払元銀行": "",
        "備考": "クレジットカード払い(楽天振込CSV対象外)",
    })

    # 口座振替・借入返済など、口座マスタに無い支払メタのみの取引先も
    # ダッシュボード(支払の見える化)のためマスタに追加する。
    # 支払方法が空の行(スプレッドシートの集計欄など)は除外。
    existing_names = {norm_name(x["会社名"]) for x in rows}
    for key, m in meta.items():
        if key in used_meta or key in existing_names:
            continue
        if not m.get("支払方法", "").strip():
            continue
        rows.append({
            "会社名": m["会社名"], "別名": "", "NE仕入先cd": "",
            "支払区分": "銀行振込", "科目": m.get("科目", ""),
            "支払方法": m.get("支払方法", ""), "支払日": m.get("支払日", ""),
            "銀行": "", "支店": "", "銀行番号": "", "支店番号": "", "預金種目": "",
            "口座番号": "", "受取人口座名": "", "顧客番号": "",
            "固定額": parse_fixed_amount(m.get("備考", "")),
            "除外フラグ": "", "ルール": "", "支払元銀行": m.get("銀行", ""),
            "備考": m.get("備考", ""),
        })

    fields = ["会社名", "別名", "NE仕入先cd", "支払区分", "科目", "支払方法", "支払日",
