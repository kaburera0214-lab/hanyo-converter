...
運用ルール（2026-07-16 ユーザー確定・シンプル設計）:
  ...
  据え置きルールは無し（下代が値下げなら価格も下がり得る。目標利益率は必ず確保される）。
    - 目標利益率価格: 利益率がちょうど目標値（既定15%）に着地する価格（calc.target_price。
      3980円未満の送料込み換算はこちらでは考慮する）
    - 値上げ率価格: **送料を含まない現販売価格**×(新下代÷旧下代)。
      ※送料込みベースに掛けると送料にも値上げ率が乗ってしまう（例: 1100円×30%は
        1430円が正で、(1100+880)×1.3−880=1694円は誤り）ため価格そのものに掛ける
...
def _ratio_price(ctx, params, factor):
    """現販売価格（送料を含まない商品価格）に倍率を掛ける。"""
    return calc.excel_round(ctx["現販売価格"] * factor)
...
def rule_markup_percent(ctx, params):
    """入力CSVに「値上げ率」列（%）があれば現販売価格×(1+率)で決める。"""
